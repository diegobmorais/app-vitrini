  ...
  <div class="p-6">
    ...
    <div class="bg-white p-4 rounded-lg shadow mb-6">
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <select v-model="filters.status" @change="applyFilters" class="border border-gray-300 rounded-md p-2">
        ...
        <input v-model="filters.date" @change="applyFilters" type="date" class="border border-gray-300 rounded-md p-2"
          placeholder="Filtrar por data" />

        <select v-model="filters.service_id" @change="applyFilters" class="border border-gray-300 rounded-md p-2">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="appointment in appointments" :key="appointment.id" class="hover:bg-gray-50">
              <td class="px-6 py-4 whitespace-nowrap">
                ...
                <!-- <div v-if="appointment.user.name" class="text-xs text-gray-400">{{ appointment.user.name }}</div> -->
              </td>
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                {{ formatDate(appointment.time_slot.slot_date) }}<br>
                <span class="text-xs">{{ formatTime(appointment.time_slot.start_time) }}</span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                <span
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                <button @click="viewDetails(appointment)" class="text-indigo-600 hover:text-indigo-900 mr-3">
                  Detalhes
                </button>
                <!-- <button v-if="appointment.status !== 'completed' && appointment.status !== 'cancelled'"
                  @click="editAppointment(appointment)" class="text-blue-600 hover:text-blue-900">
                  Editar
                </button> -->
              </td>
            </tr>
      ...
      <!-- Paginação -->
      <Pagination :current-page="pagination.current_page" :totalPages="pagination.last_page" :total="pagination.total"
                ...
                <div class="mt-4 space-y-3">
                  <div class="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p class="mt-1 text-sm text-gray-900">{{ selectedAppointment?.user.name }}</p>
                    </div>
                    <div>
                      <label class="block text-sm font-medium text-gray-700">Telefone</label>
                      <p class="mt-1 text-sm text-gray-900">{{ selectedAppointment?.user.phone }}</p>
                    </div>
                    <div v-if="selectedAppointment?.client_email">
                      <label class="block text-sm font-medium text-gray-700">E-mail</label>
                      <p class="mt-1 text-sm text-gray-900">{{ selectedAppointment.user.email }}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p class="mt-1 text-sm text-gray-900">
                        {{ formatDate(selectedAppointment.time_slot.slot_date) }} às {{
...
const handlePageChanged = (page) => {
  appointmentStore.fetchAppointments(page)
}
</script>