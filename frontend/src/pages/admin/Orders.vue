                ...
                <td class="px-6 py-4">
                  <div class="flex items-center">
                    <div class="flex-shrink-0 h-8 w-8 bg-gray-200 rounded-full flex items-center justify-center">
                      <span v-if="!order.customer.avatar" class="text-gray-500 font-medium text-xs">{{ getInitials(order.customer.name) }}</span>
        ...
        <div class="bg-white px-4 py-3 border-t border-gray-200 sm:px-6">
          <div class="flex justify-between items-center">
            <div class="text-sm text-gray-700">
              Mostrando <span class="font-medium">{{ filteredOrders.length }}</span> de <span class="font-medium">{{ orders.length }}</span> pedidos
            </div>
      ...
      updateOrderStatus(orderId, newStatus) {
        ...
        if (order) {
          ...
          this.$store.dispatch('notifications/add', {
            type: 'success',
            message: `Status do pedido #${order.number} atualizado para ${this.getStatusLabel(newStatus)}`
          });
        }
      }
    }
