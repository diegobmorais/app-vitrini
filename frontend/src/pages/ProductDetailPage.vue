<template>
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
          ...
          <li v-if="product && product.category_name">
            <div class="flex items-center">
              ...
              <router-link 
                :to="{ name: 'category', params: { slug: product.category_slug } }" 
                class="ml-1 text-gray-500 hover:text-primary-600 md:ml-2"
              >
          ...
          <div class="md:w-1/2 p-6 flex flex-col">
            ...
            <div class="flex items-center mb-4">
              <div class="flex text-yellow-400">
                <svg v-for="i in 5" :key="i" class="w-5 h-5" :class="{ 'text-gray-300': i > Math.round(product.rating) }" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div class="border-t border-gray-200 px-6 py-6">
          ...
          <div v-if="activeTab === 'reviews'" class="py-4">
            <div v-if="product.reviews && product.reviews.length > 0">
              <div v-for="(review, index) in product.reviews" :key="index" class="mb-4 pb-4 border-b border-gray-200 last:border-b-0">
  ...
  export default {
    ...
    setup(props) {
      ...
      const addToCartRelated = (product, quantity) => {
        store.dispatch('cart/addToCart', { product, quantity })
      }
      
      // Lifecycle hooks
      onMounted(() => {
        store.dispatch('products/fetchProduct', props.slug)
      })
      
      // Watch for changes in product or route
      watch(() => props.slug, (newSlug) => {
        if (newSlug) {
          store.dispatch('products/fetchProduct', newSlug)
          quantity.value = 1
