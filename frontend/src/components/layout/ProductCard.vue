  ...
  <div class="bg-white rounded-lg shadow-sm overflow-hidden transition-all duration-300 hover:shadow-md group">
    ...
    <div class="p-4">
      ...
      <div class="mt-2 flex items-center text-xs">
        <span v-if="product.stock_status === 'in_stock'" class="text-green-600 flex items-center">
          <i class="fas fa-check-circle mr-1"></i> Em estoque
        </span>
        <span v-else-if="product.stock_status === 'low_stock'" class="text-yellow-600 flex items-center">
      ...
      <!-- Add to Cart Button -->
      <button @click="$emit('add-to-cart', product, 1)"
...
function toggleWishlist() {
  ...
  // Aqui você faria a chamada real para API se desejar, ex:
  // if (isInWishlist.value) {
  //   axios.post('/api/wishlist', { product_id: props.product.id })
  // } else {
  //   axios.delete(`/api/wishlist/${props.product.id}`)
  // }
